...
local plugin_name = "crime_search_result"
...
function _M.access(conf, ctx)
    ...
    -- Rename task_id to cs_id
    if data.task_id ~= nil then
        -- If task_id is present, rename it to cs_id
        data.cs_id = data.task_id
        data.task_id = nil

        -- Re-encode and set the modified request body
        ngx.req.set_body_data(json.encode(data))
    else
        -- If task_id is not present, return an error
...
function _M.body_filter(conf, ctx)
    ...
    if eof then
        ...
        if input_data["cs_id"] ~= nil then
            input_data["task_id"] = input_data["cs_id"]
            input_data["cs_id"] = nil
        end
        ...
        if data and data.message then
            local msg_data = data.message or ""

   
            -- Trim whitespace just in case
            msg_data = tostring(msg_data):match("^%s*(.-)%s*$")

            if msg_data == "cs_id not found" then
                result["response_message"] = "task_id not found"
                result["result"] = {}
                result["billable"] = "False"
                result["success"] = "False"
            elseif msg_data=="Invalid request body" then
                result["result"] = {}
                result["billable"] = "False"
                result["success"] = "False"
            elseif msg_data=="task_id not found" then
                result["result"] = {}
                result["billable"] = "False"
                result["success"] = "False"
