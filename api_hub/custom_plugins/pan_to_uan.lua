...
function _M.body_filter(conf, ctx)
    ...
    if eof then
        ...
        -- Promote specific keys from result to the top level
        if type(result.result) == "table" then
            local keys_to_promote = { "uan", "dob", "memberId", "name" }
            for _, key in ipairs(keys_to_promote) do
                if result.result[key] then
