...
local function sanitize_string(value)
    ...
    -- value = value:gsub('"', '\\"')
     ...
     -- Trim whitespace
     -- Remove control characters (just for safety)
    value = value:gsub("[%z\1-\31\127]", "")

    -- Fix invalid backslashes: \ followed by non-valid escape character
    value = value:gsub("\\([^\"\\/bfnrtu])", "\\\\%1")

    -- Also escape remaining valid ones (if needed)
    value = value:gsub("\\", "\\\\")
    value = value:gsub('"', '\\"')
    value = value:gsub("\r", "")
    value = value:gsub("\n", " ")
...
-- local function deep_sanitize(data)
--     if type(data) == "table" then
--         for k, v in pairs(data) do
--             data[k] = deep_sanitize(v)
--         end
--         return data
...
local function address_classification(address)
    ...
    core.log.warn("Address classification decoded result: ", core.json.encode(decoded))
    return decoded, nil
end
...
function _M.access(conf, ctx)
    ...
    for _, node in ipairs(conf.nodes) do
        ...
        while (ngx.now() - start_time) < 80 do
    ...
    -- Log the raw upstream response for debugging
    core.log.warn("Raw upstream response body:", last_resp.body)
    -- Sanitize response: remove control characters
    local sanitized_body = type(last_resp.body) == "string" and last_resp.body:gsub("[%z\1-\31\127]", "") or last_resp.body
    local decoded_body = core.json.decode(sanitized_body)
    if not decoded_body then
        core.log.warn("Upstream response could not be decoded as JSON after sanitization. Raw:", sanitized_body)
        return last_resp.status, "Invalid JSON from upstream: " .. tostring(sanitized_body)
    end
    ...
    if found then
        ...
        if decoded_body.result and type(decoded_body.result) == "table" then
            for _, v in pairs(decoded_body.result) do
                ...
                if not inputAddressData.result or not resultAddressData.result then
                    core.log.warn("Missing result field in address classification data",
                        " | inputAddressData: ", core.json.encode(inputAddressData),
                        " | resultAddressData: ", core.json.encode(resultAddressData))
                    goto continue
                end
            ...
            core.log.warn("No result key found in response body or result is not a table")
        end
        ...
        deep_sanitize(decoded_body)
        escape_backslashes(decoded_body)
    ...
    if found then
        ...
        if decoded_body.result then
            for _, v in pairs(decoded_body.result) do
                ...
                -- Check .result exists before accessing
                if not inputAddressData.result or not resultAddressData.result then
                    core.log.warn("Missing result field in address classification data",
                        " | inputAddressData: ", core.json.encode(inputAddressData),
                        " | resultAddressData: ", core.json.encode(resultAddressData))
