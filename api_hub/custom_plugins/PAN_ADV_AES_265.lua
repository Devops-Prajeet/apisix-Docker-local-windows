...
local plugin_name = "PAN_ADV_AES_265"
...
function _M.check_body(ctx, required_key_1, json_body)
...
function _M.access(conf, ctx)
    ...
    local status, responses = _M.check_body(ctx, required_key_1,json_data)
...
function _M.body_filter(conf, ctx)
    ...
    if chunk ~= "" then
        ...
        if newData.input.pan_number then
         ...
         newData =  sorted_json(newData)

        --  newData = cjson.decode(newData)
        ...
        -- ngx.arg[1] = sorted_json(responseData,key_order)
    end
