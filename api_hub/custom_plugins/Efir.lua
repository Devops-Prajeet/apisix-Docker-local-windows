    ...
    if found then
        ...
        if decoded_body.result then
            for _, v in pairs(decoded_body.result) do
                ...
                v.house_number = houseScore >= 50 and "EXACT_MATCH" or (houseScore >= 20 and "PARTIAL_FUZZY" or "NO_MATCH")
                v.landmark = localityScore >= 50 and "EXACT_MATCH" or (localityScore >= 20 and "PARTIAL_FUZZY" or "NO_MATCH")
                v.area_type = areaTypeScore >= 50 and "EXACT_MATCH" or (areaTypeScore >= 20 and "PARTIAL_FUZZY" or "NO_MATCH")
                if areaTypeScore >= 50 then
